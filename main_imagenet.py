...
# Create data loaders:
data_train = data_loader.data_loader_imagenet(annotations_train, opts)
data_val = data_loader.data_loader_imagenet(annotations_val, opts)
...
with tf.Session(config=tf_config) as sess:
    ...
    if opts.train:
        if opts.modelname == 'cnn_emotic_1':
            ...
            cnn_actions_2.train(sess, saver, opts, dircase, data_train, data_val, gradients)
        else:
            tools.error('<main_imagenet> modelname not recognized.')
